...
export const Sidebar = () => {
  ...
  return (
    <div className="h-screen p-2 bg-white dark:bg-bg-dark transition-all duration-300 flex flex-col">
      {/* LOGO */}
      <div className=" h-8  w-8 flex justify-center items-center rounded-full bg-blue-100 text-violet-600 font-bold text-xs p-2 m-2">SF</div>
      {/* NAVEGACION */}
      <nav className="flex-1 flex-col flex gap-2 items-center">
        {
            links.map((item, index) => {
                return(
                    <NavLink key={index} to={item.to} className={({isActive}) => `flex items-center gap-3 p-2 rounded-lg font-semibold hover:bg-gray-100 dark:hover:bg-primary/10 dark:hover:text-violet-600 transition-all w-full justify-center sm:justify-start 
                    ${
                        isActive ? "text-violet-600 dark:text-white" : "text-gray-600 dark:text-gray-400"
                    } `}>
